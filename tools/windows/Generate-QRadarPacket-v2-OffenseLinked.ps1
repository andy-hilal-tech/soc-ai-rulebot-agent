# Generate-QRadarPacket-v2-OffenseLinked.ps1
#
# Purpose:
...
function Convert-MsEpochToUtc {
    ...
    if ($null -eq $Ms -or :IsNullOrWhiteSpace("$Ms")) {
        return ""
    }
...
function Invoke-Qradar {
    ...
    if (-not :IsNullOrWhiteSpace($RangeHeader)) {
        $args += "-H"
...
function Get-QradarSearchId {
    ...
    if ($props -contains "search_id" -and -not :IsNullOrWhiteSpace("$($SearchResponse.search_id)")) {
        return [string]$SearchResponse.search_id
    }

    if ($props -contains "cursor_id" -and -not :IsNullOrWhiteSpace("$($SearchResponse.cursor_id)")) {
...
function Wait-QradarSearch {
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        Write-Host "Ariel search $SearchId status: $state" -ForegroundColor DarkYellow
...
function Invoke-DistributionQuery {
    ...
    if (:IsNullOrWhiteSpace($GroupByClause)) {
...
Write-Host ""
Write-Host "Running offense-linked Ariel aggregation queries..." -ForegroundColor Cyan
...
$payloadSummaryParts += "SourceIpDistribution=" + (Compact-Json $topSourceIps)
$payloadSummaryParts += "DestinationIpDistribution=" + (Compact-Json $topDestinationIps)
$payloadSummaryParts += "QidDistribution=" + (Compact-Json $topQids)
$payloadSummaryParts += "UsernameDistribution=" + (Compact-Json $topUsernames)
$payloadSummaryParts += "LogSourceDistribution=" + (Compact-Json $topLogSources)
$payloadSummaryParts += "CategoryDistribution=" + (Compact-Json $topCategories)
$payloadSummaryParts += "QidLogSourceCategoryDistribution=" + (Compact-Json $qidLogSourceCategory)
$payloadSummaryParts += "SampleEvents=" + (Compact-Json $sampleEvents)
...
$template += "top_source_ips: $(Compact-Json $topSourceIps)`r`n"
$template += "top_destination_ips: $(Compact-Json $topDestinationIps)`r`n"
$template += "top_qids: $(Compact-Json $topQids)`r`n"
$template += "top_usernames: $(Compact-Json $topUsernames)`r`n"
$template += "top_log_sources: $(Compact-Json $topLogSources)`r`n"
$template += "top_categories: $(Compact-Json $topCategories)`r`n"
$template += "qid_logsource_category_distribution: $(Compact-Json $qidLogSourceCategory)`r`n"
$template += "sample_events: $(Compact-Json $sampleEvents)`r`n"
